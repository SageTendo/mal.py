...
class Client:
    ...
    def _generate_verifier_challenger_pair(self, length: int, method: str = ""):
        """
        ...
        """
        verifier = self._generate_verifier(length)
        return verifier, self._generate_challenge(verifier, method)

    def _generate_verifier(self, length: int = 128):
        """
    ...
    async def get_access_token(self, authorization_code: str, code_verifier: str) -> Auth:
        """
    ...
    async def get_anime_details(self, *, anime_id: str, token: Optional[str] = None) -> Anime:
        """
